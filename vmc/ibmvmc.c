...
static inline long h_set_part_mode(struct partition_policy_struct *pp_buffer)
{
	long rc = 0;

	pr_debug("ibmvmc: h_set_part_mode\n");
	/* Call to request alpha mode state change from phyp */
	rc = plpar_hcall_norets(H_POLICY_AND_STATE, virt_to_phys(pp_buffer),
		sizeof(pp_buffer));
	pr_debug("ibmvmc: h_set_part_mode rc = 0x%lx\n", rc);
		...
		u32 __user *ret_vmc_index)
{
	...
	pr_debug("ibmvmc: requestvmc: H_REQUEST_VMC\n");
	/* Call to request the VMC device from phyp*/
	...
	if (rc == H_FUNCTION) {
		long ret;
		struct partition_policy_struct *pp_buffer;

		/* Build out partition policy structure for hcall */
		memset(&pp_buffer, 0, sizeof(pp_buffer));
		pp_buffer->identifier = HPPS_SET_PARTITION_MODE;
		pp_buffer->read_write = WRITE_FLAG;
		pp_buffer->data[0] = VMC_SET_ALPHA;

		pr_debug("ibmvmc: requestvmc: Attempt to fall back to alpha mode\n");
		/* Set partition to alpha mode to receive VMC device from phyp*/
		ret = h_set_part_mode(pp_buffer);
		pr_debug("ibmvmc: requestvmc: Alpha req rc = 0x%lx\n", rc);
		if (ret == H_BUSY) {
			return -EBUSY;
		} else if (ret == H_AUTHORITY) {
			pr_debug("ibmvmc: requestvmc: hypervisor denied alpha mode request\n");
			return -EPERM;
		}

		/* Set dummy drc index to indicate alpha mode vmc */
		vmc_drc_index = 0xffff;

	} else if ((rc == H_BUSY) || H_IS_LONG_BUSY(rc)) {
		return -EBUSY;
	} else if (rc == H_AUTHORITY) {
		pr_debug("ibmvmc: requestvmc: hypervisor denied vmc request\n");
		return -EPERM;
